...
export function createListStatusManager(initOption: ListStatusManagerInitOptionType) {
    return new StatusManager({
        ...
        mutations: {
            ...
            searchNode(state: any[], payload: { result: Object[], searchData: { [propName: string]: any } }) {
                state.forEach((e: any,index:number) => {
                    ...
                    if (ok) payload.result.push({
                        index,
                        node:e
                    });
                })
            },
            editNode(state:any[],payload:{position:number,path:string[],value:any}){
            ...
            editNodeByKey(state:any[],payload:{key:string,keyIndex?:string,path?:string[],value:any}){
